...
execute @a[tag=upgrade_red_protection] ~~~ tellraw @s { "rawtext" : [ { "text" : "§l商店 >> §d§l升级成功 §r§6团队·保护" } ] }
execute @a[tag=upgrade_red_protection] ~~~ tellraw @a[scores={"分队"=1}] { "rawtext" : [ { "text" : "§l商店 · 队伍升级 >> §e " } ,{ "selector" :  "@e[type=player,x=4,y=186,z=56,r=3,c=1,scores={分队=1}]"},{ "text" : "§f§l 为您的队伍升级了 保护" }] }
