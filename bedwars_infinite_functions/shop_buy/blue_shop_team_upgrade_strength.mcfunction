...
execute @a[tag=upgrade_blue_strength] ~~~ tellraw @s { "rawtext" : [ { "text" : "§l商店 >> §d§l升级成功 §r§6团队·锋利" } ] }
execute @a[tag=upgrade_blue_strength] ~~~ tellraw @a[scores={"分队"=2}] { "rawtext" : [ { "text" : "§l商店 · 队伍升级 >> §e " } ,{ "selector" :  "@e[type=player,x=-4,y=186,z=-55,r=3,c=1,scores={分队=2}]"},{ "text" : "§f§l 为您的队伍升级了 锋利" }] }
